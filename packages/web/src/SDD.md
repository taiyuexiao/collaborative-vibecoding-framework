...
# SDD: E2E 浏览器真人路径测试（修复记录）

对 7300 实例做浏览器 UI 真人路径测试，发现并修复两个产品缺陷：

1. **报错（UI 真 bug）**：服务端数据重置后，浏览器残留的失效 token 导致所有写操作 401，页面停留在「全部按钮都失败」的看板，无任何恢复路径。
   **解决**：api.ts 在 401 且本地有 token 时清除凭证并 reload 回注册门。浏览器验证：点创建 → 自动回注册门 → 重新注册后全流程可用。
2. **报错（UI 真 bug）**：server 重启后页面 WebSocket 死亡且永不重连，实时刷新静默失效。
   **解决**：App.tsx WS 加指数退避自动重连（1s→15s 封顶），重连成功立即 refresh。浏览器验证：API 建任务 → 不刷新页面卡片自动出现。
3. **报错（产品逻辑）**：日报「任务动态（0）」——digest 过滤了已被蒸馏器消费的事件；「被消费≠没发生」。
   **解决**：digest.ts 去掉 consumedAt 条件，日报反映活动历史。UI 复核：任务动态（11）。
